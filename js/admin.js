...
StoreAdmin.bineForms = function(){
	...
	var addStoreForm=$("form#change-store"); /*added for ninja*/
    addStoreForm.submit(function(e){
        e.preventDefault();
        var submittedForm=$(this);
        console.log(submittedForm);
        var newStoreName=submittedForm.find("input[name='store-name']").val();
        $.post("/store",{"STORE":newStoreName}, function(result){
            if (result["STATUS"] == "ERROR"){
				alert(result["MSG"]);
			}else{
				alert("Store name was successfully changed to"+result["STORE"])
			}
        }, "json");
        return false;
    });
...
StoreAdmin.deleteCategory = function(deleteBtn){
	...
	$.ajax({
		...
		if (result["STATUS"]=="ERROR"){ /*Added if else condition as I think it was forgotten, since it does not notiify the admin about errors*/
		    alert(result["MSG"])        /*Added if else condition as I think it was forgotten, since it does not notiify the admin about errors*/
		}                               /*Added if else condition as I think it was forgotten, since it does not notiify the admin about errors*/
		else{                           /*Added if else condition as I think it was forgotten, since it does not notiify the admin about errors*/
